...
Status decode_magic_string(char *magic_string, DecodeInfo *decoInfo)
{
    ...
    char str[3];
    scanf("%s", str);
    if ((strcmp(str, decoInfo->image_data)) == 0)
    {
        return e_success;
    }
    else
...
Status do_decoding(DecodeInfo *decoInfo)
{
    ...
    if (decode_secret_file_extn_size(sizeof(int), decoInfo->fptr_src_image) == e_success)
    {
    ...
    // DECODE EXTENSION
    if (decode_secret_file_extn(".txt", decoInfo) == e_success)
    {
    ...
    if (decode_secret_file_size(decoInfo->size_secret_file, decoInfo) == e_success)
    {
    ...
    // DECODE FILE DATA
    if (decode_secret_file_data(decoInfo) == e_success)
    {
